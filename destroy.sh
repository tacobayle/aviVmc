...
if [ -f "data.json" ]; then
  echo ""
  echo "TF refresh..."
  terraform refresh -var-file=sddc.json -var-file=ip.json -var-file=data.json -var-file=EasyAviLocation.json -var-file=vCenterIp.json -no-color
  echo ""
  echo "TF destroy..."
  terraform destroy -auto-approve -var-file=sddc.json -var-file=ip.json -var-file=data.json -var-file=EasyAviLocation.json -var-file=vCenterIp.json -no-color
else
  echo ""
  echo "TF refresh..."
